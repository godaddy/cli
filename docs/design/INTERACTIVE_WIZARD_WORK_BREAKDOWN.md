...
- [ ] Add `dialoguer`, `console`, `indicatif` to gddy Cargo.toml
- [ ] Create `rust/src/domain/register/` directory structure: `mod.rs`, `wizard.rs`, `steps/{mod,discovery,options,review,execute}.rs`
- [ ] Define `WizardState` struct (domain, available, period, privacy, auto_renew, nameservers, quote_token, price, etc.)
- [ ] Define `StepResult` enum (Continue, Back, Cancel) and `WizardStep` trait
- [ ] Implement `run_wizard()` step sequencer with forward/back navigation
- [ ] Define `StepContext` (credential, env, debug, is_interactive, term)
- [ ] Implement Discovery step: prompt for domain, call `/v3/domains/available`, show suggestions if taken, progressive pagination (5→15→25→50)
- [ ] Implement Options step: period Select, privacy Confirm, auto-renew Confirm, custom NS Input
- [ ] Implement Review step: call quote API, fetch agreements, display order summary, confirm prompt
- [ ] Implement Execute step: call register API, show spinner, display success + next_actions
- [ ] Create `RegisterArgs` struct with all CLI flags (`--period`, `--privacy`, `--agree`, `--confirm`, `--non-interactive`, etc.)
- [ ] Implement TTY detection + non-interactive fallback (map flags → WizardState → execute directly)
- [ ] Wire into domain group in `domain/mod.rs`
- [ ] Implement human-friendly output for interactive mode (colored summary, not JSON)
