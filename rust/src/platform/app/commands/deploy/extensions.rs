...
use crate::platform::app::extension::{
    BundleCleanup, BundleOptions, ExtensionType, Finding, Severity, bundle_extension, is_blocked,
    repo_root_from_cwd, require_extension_source_file, resolve_extension_paths, scan_bundle,
    scan_extension,
};
    ...
    if let Some(exts) = &config.extensions {
        ...
        for e in &exts.checkout {
            result.push(ExtensionDeploy {
                ...
                handle: e.handle.clone(),
                source: e.source.clone(),
                ext_type: ExtensionType::Checkout,
                targets: e.targets.iter().map(|t| t.target.clone()).collect(),
            });
        }
...
fn resolve_upload_targets(ext_type: ExtensionType, targets: &[String]) -> Vec<Option<String>> {
    match ext_type {
        ExtensionType::Blocks => vec![Some("blocks".to_owned())],
...
pub(super) async fn deploy_extension(
    ...
    let (ext_dir, source) = resolve_extension_paths(&repo_root, &ext.handle, &ext.source, ext_name)
        .map_err(super::super::validation_err)?;
    require_extension_source_file(ext.handle.as_str(), ext_name, &source)
        .map_err(super::super::validation_err)?;
    ...
    let pre_report = scan_extension(&ext_dir).map_err(|e| {
    ...
    let _bundle_cleanup = BundleCleanup::new(&bundle);
    ...
    let findings = scan_bundle(&content, source_display);
...
mod tests {
    ...
    use crate::platform::app::extension::ExtensionType;
    ...
    fn collect_extensions_maps_embed_extensions() {
        ...
        assert_eq!(deploys[0].ext_type, ExtensionType::Embed);
    ...
    fn collect_extensions_maps_checkout_extensions() {
        ...
        assert_eq!(deploys[0].ext_type, ExtensionType::Checkout);
    ...
    fn collect_extensions_maps_blocks_extension_with_fixed_name_and_handle() {
        ...
        assert_eq!(deploys[0].ext_type, ExtensionType::Blocks);
        assert!(deploys[0].targets.is_empty());
    }
