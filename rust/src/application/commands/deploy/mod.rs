...
pub(super) fn command() -> RuntimeCommandSpec {
    RuntimeCommandSpec::new_typed_streaming::<DeployArgs, _, _>(
        ...
        .with_long(
            ...
            extensions with esbuild, run the security scanner (rules \
            SEC101–SEC115) on each bundle, then upload the artifacts to the \
            latest release of the named application. Progress is streamed as \
            JSON events. A release must exist before deploying; create one with \
