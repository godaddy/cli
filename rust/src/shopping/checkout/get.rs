...
const HUMAN_VIEW_ID: &str = "shopping-checkout-get";
...
fn human_response(checkout: &Value) -> Value {
...
fn money(amount: i64, currency: Option<&str>) -> String {
    format!("{} {:.2}", currency.unwrap_or(""), amount as f64 / 100.0)
        .trim()
        .to_owned()
}
...
fn render_human(checkout: &Value) -> String {
        ...
        .flatten()
    {
        ...
        output.push_str(&format!(
            "- {label}: {}\n",
            money(amount, checkout.get("currency").and_then(Value::as_str)),
        ));
    }
