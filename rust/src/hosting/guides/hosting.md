...
**Environments** — each app has a PREVIEW environment and a PUBLISH environment. The CLI calls this `variant`. Uploading source makes your app available on its PREVIEW URL for testing. `deployment publish`, when successful, makes your app available on its public PUBLISH URL.
